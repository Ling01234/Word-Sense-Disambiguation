...
class WSDInstance:
    def __init__(self, my_id, lemma, context, index):
        ...
        self.lemma = lemma.decode('utf-8')
        # lemma of all the words in the sentential context
        self.context = ' '.join([byte.decode('utf-8') for byte in context])
...
def get_instances():
    ...
    # ic(dev_instances['d001.s001.t002'].__str__())  # number of dev instances
    # ic(len(dev_instances))  # number of dev instances
    # ic(len(test_instances))  # number of test instances
